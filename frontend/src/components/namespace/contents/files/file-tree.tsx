...
import {Directory, File, sortDir, sortFile} from "../files/file-manager";
import {getIcon} from "./icon";
import styled from "@emotion/styled";

interface FileTreeProps {
  rootDir: Directory;   // root directory
  selectedFile: File | undefined;   // currently selected file
  onSelect: (file: File) => void;  // event triggered when selection changes
}
...
const FileDiv = ({file, icon, selectedFile, onClick}: {
  file: File | Directory; // current file
  icon?: string;          // icon name
  selectedFile: File | undefined;     // selected file
...
const DirDiv = ({directory, selectedFile, onSelect}: {
  ...
  selectedFile: File | undefined;    // selected file
  onSelect: (file: File) => void;  // click event
}) => {
